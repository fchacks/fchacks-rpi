...
apt-get --force-yes -y install locales console-common ntp openssh-server less vim parted raspberrypi-kernel raspberrypi-bootloader network-manager pi-bluetooth code-server figlet python-rpi.gpio python3-rpi.gpio python3-pip
echo 'start boot'
echo 'dtoverlay=vc4-fkms-v3d' >> /boot/config.txt
echo 'max_framebuffers=2' >> /boot/config.txt
echo 'arm_64bit=1' >> /boot/config.txt
echo '#!/bin/sh -e' > /etc/rc.local
echo 'su pi -c \"code-server --auth none --bind-addr 0.0.0.0:8080 & \" ' >> /etc/rc.local
...
apt-key add raspberrypi.gpg.key
apt-get update 
apt-get -y --force-yes install binutils ca-certificates wget raspi-config libraspberrypi-* nano git pi-bluetooth firmware-brcm80211 gnupg2 network-manager figlet
figlet fchacks > /etc/motd
...
chmod +x /usr/bin/rpi-update
UPDATE_SELF=0 SKIP_BACKUP=1 /usr/bin/rpi-update
apt-get --force-yes -y install locales console-common ntp openssh-server less vim parted figlet
figlet fchacks > /etc/motd
...
if [ "$deb_arch" == "arm64" ]; then 
    ...
    echo "deb http://fchacks.org/archive/ $deb_release main" >> etc/apt/sources.list
elif [ "$deb_arch" == "armhf" ]; then 
    echo "deb http://archive.raspberrypi.org/debian $deb_release main ui" >> etc/apt/sources.list
elif [ "$deb_arch" == "armel" ]; then 
